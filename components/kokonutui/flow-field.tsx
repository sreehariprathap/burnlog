...
export interface FlowFieldProps {
  ...
  /** RGB triplet string (e.g. "249, 249, 249"), overrides the theme's built-in
   * background — lets callers switch between a light and dark background
   * instead of being locked to the theme's fixed dark canvas. */
  backgroundFill?: string;
  /** Overrides the theme's particle hue start/range/saturation/lightness —
   * lets callers tint particles to their own brand color instead of the
   * fixed aurora/ember/ocean palettes. */
  hueStart?: number;
  hueRange?: number;
  saturation?: number;
  lightness?: number;
...
export default function FlowField({
  ...
  useEffect(() => {
    ...
    const cfg: ThemeConfig = {
      ...preset,
      bg: backgroundFill ?? preset.bg,
      hueStart: hueStart ?? preset.hueStart,
      hueRange: hueRange ?? preset.hueRange,
      saturation: saturation ?? preset.saturation,
      lightness: lightness ?? preset.lightness,
    };
  ...
  }, [theme, density, backgroundFill, hueStart, hueRange, saturation, lightness]);

  const bgColor = backgroundFill ?? THEMES[theme].bg;
