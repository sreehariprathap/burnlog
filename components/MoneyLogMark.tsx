...
// ambient theme (`text-primary` would render orange when this is shown
// from a BurnLog page, since `.app-moneylog` isn't applied there and
// --primary still resolves to BurnLog's palette). Plain letterform to
// match the other apps' marks — bold glyph, no background badge.
export function MoneyLogMark({ size = 20, className }: MoneyLogMarkProps) {
  const color = useAppSearchColor('moneylog');
  return (
    <span
