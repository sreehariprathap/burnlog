...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = (await request.json()) as { autoRotate?: boolean };
    if (typeof body.autoRotate !== 'boolean') {
      return NextResponse.json({ error: 'autoRotate must be a boolean' }, { status: 400 });
    }
    ...
    const { error: updateError } = await admin
      ...
      .update({ autoRotate: body.autoRotate })
      .eq('id', id);
    if (updateError) {
