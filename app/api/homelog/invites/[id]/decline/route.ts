    ...
    try {
      ...
      if (inviter) {
        ...
        await sendPushToUser(admin, inviter.userId, {
          ...
          message: `${declinerName} declined your household invite.`,
          url: '/homelog',
        });
      }
    } catch (pushError) {
      console.error('household invite-declined push error:', pushError);
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('decline household invite error:', error);
